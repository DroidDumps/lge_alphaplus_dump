...
function do_copy_folder() {
  ...
  for file in $(find ${source_folder} -type f -name "*.*"); do
    ...
    #log -p i -t preload_copy "do_copy_folder : temp_name: ${temp_name}, dest_name: ${dest_name}, dest_folder: ${dest_folder}"
...
if [ $# -eq 1 ]; then
  ...
  log -p e -t preload_copy "Usage: preload_copy.sh <system_other-mount-point>"
  exit 1
fi
